...
const Home = () => {
  const projects = [
    ...
    
    {
      name: "Foodie Fest",
      description:
        "A food ordering website build on react libary where we can see foods as per the categories and add o cart and order",
  ...
  return (
    <div id="home" className="w-full my-4 bg-zinc-900 p-1 md:p-4 ">
      ...
      <div
        id="project-heading"
        className="w-[80vw] sm:w-[60vw] md:w-[40vw] lg:w-[25vw] text-center rounded-md shadow-md shadow-white p-3 uppercase mx-auto border border-zinc-100 mt-40 sm:mt-40 md:mt-28 lg:mt-32"
      >
        Projects
      </div>
      ...
      <div id="projects" className="w-full my-4 relative">
        {projects.map((p, i) => {
          return <Project key={i} p={p} />;
        })}
      </div>

      <div
        id="project-heading"
        className=" w-[25vw] text-center rounded-md shadow-md  shadow-white p-3 uppercase mx-auto border border-zinc-100 mt-32 md:mt-28"
      >
        Skills
      </div>
